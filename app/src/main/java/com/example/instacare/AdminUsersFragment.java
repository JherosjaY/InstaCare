...
public class AdminUsersFragment extends Fragment {
    ...
    private void handleSuspend(User user) {
        new Thread(() -> {
            if (user.isSuspended) {
                db.userDao().unsuspendUserById(user.uid);
            } else {
                db.userDao().suspendUserById(user.uid);
            }
        ...
        new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("Delete", (d, w) -> {
                    new Thread(() -> {
                        ...
                        // 1. Cleanup Alert Notifications (linked to Alerts)
                        List<Integer> alertIds = db.emergencyAlertDao().getAlertIdsByUser(userId);
                        if (alertIds != null) {
                        ...
                        // 2. Cleanup User-linked records (Strict UID isolation)
                        ...
                        requireActivity().runOnUiThread(() -> {
                            Toast.makeText(requireContext(), "User and associated data deleted", Toast.LENGTH_SHORT).show();
                            loadUsers();
                        });
                    }).start();
                })
