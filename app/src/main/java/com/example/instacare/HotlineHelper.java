...
public class HotlineHelper {
    ...
    private static final String[][] HOTLINES = {
        ...
        // Fire
        {"Valencia BFP", "0926-190-3020", "Fire Department", "ic_alarm_active"},
        // Disaster Response
    ...
    public static void populateCategorized(Context ctx, LinearLayout container, String filterCategory, boolean showHeaders, OnHotlineClickListener listener) {
        container.removeAllViews();

        float density = ctx.getResources().getDisplayMetrics().density;

        for (String category : CATEGORIES) {
            // If a filter is applied, skip other categories
            if (filterCategory != null && !filterCategory.equals(category)) continue;
            ...
            for (String[] hotline : HOTLINES) {
                ...
                card.setOnClickListener(v -> {
                    // Log the Emergency Call (Hardened via UID)
                    SessionManager sessionManager = SessionManager.getInstance(ctx);
                    int userId = sessionManager.getCurrentUserUid();
                    String desc = "Emergency call to " + hotline[0] + " (" + hotline[1] + ")";
                    
                    new Thread(() -> {
                        ActivityLog log = new ActivityLog("SOS", desc, userId, category, System.currentTimeMillis());
