...
public class VerifyEmailActivity extends AppCompatActivity {
    ...
    private void handleVerifyLogic() {
        ...
        if (!code.equals("1234")) {
            ...
            // Flash boxes red briefly (1s Auto-Reset)
        ...
        // Mark user as verified in DB (UID-based)
        String email = getIntent().getStringExtra("email");
        new Thread(() -> {
            if (email != null) {
                com.example.instacare.data.local.User user = com.example.instacare.data.local.AppDatabase.getDatabase(this).userDao().getUserByEmail(email);
                if (user != null) {
                    com.example.instacare.data.local.AppDatabase.getDatabase(this).userDao().verifyUserById(user.uid);
                }
            }
            
            // Sync to SessionManager for primary app flow (UID-isolated)
            com.example.instacare.SessionManager.getInstance(this).putBoolean("EMAIL_BOUND", true);
