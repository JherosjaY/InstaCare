...
public class OnboardingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // --- 5. Premium Liquid Slide & Parallax Transformer ---
        binding.viewPager.setPageTransformer((page, position) -> {
            ...
            if (position < -1) { // Way off-screen to the left
                ...
                // 1. Depth & Scale Effect
                float scaleFactor = Math.max(0.9f, 1 - Math.abs(position));
                page.setScaleX(scaleFactor);
                page.setScaleY(scaleFactor);
                page.setAlpha(1 - Math.abs(position / 2f)); // Subtle fade
                
                // 2. Parallax Effect: Move background image slower 
                android.view.View image = page.findViewById(R.id.imageOnboarding);
                if (image != null) {
                    image.setTranslationX(-position * (pageWidth / 1.5f)); 
                }

                // 3. Text Slide Effect: Smooth horizontal move
                android.view.View text = page.findViewById(R.id.textContainer);
                if (text != null) {
                    text.setTranslationX(position * (pageWidth / 3f));
                    text.setAlpha(1 - Math.abs(position));
                }

                // Ensure clipping is reset if it was previously set
        ...
        binding.nextBtn.setOnClickListener(v -> {
            ...
            if (current < items.size() - 1) {
                ...
                // Mark Onboarding as Complete using SessionManager
                SessionManager.getInstance(this).putGlobalBoolean("ONBOARDING_COMPLETE", true);
    ...
    private void smoothScrollToPageWithFakeDrag(int targetPage) {
        ...
        smoothScrollAnimator.setDuration(600); // 600ms duration for majestic switching
