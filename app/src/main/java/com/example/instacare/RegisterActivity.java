...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize Cara's voice early (Sign-up page) to eliminate first-speak delay in tutorial
    ...
    private void handleRegister() {
        ...
        new Thread(() -> {
            ...
            // Create User & Retrieve Generated UID
            com.example.instacare.data.local.User newUser = new com.example.instacare.data.local.User(username, email, password, phone, "USER");
            long generatedUid = userDao.registerUser(newUser);
            ...
            // 🎁 WELCOME NOTIFICATION (Isolated to this user)
            com.example.instacare.data.local.Notification welcomeNotice = new com.example.instacare.data.local.Notification(
                "Welcome to InstaCare!",
                "Hi " + username + "! Welcome to our community. We're committed to your safety. Please set up your Emergency Contacts to stay prepared.",
                System.currentTimeMillis(),
                false,
                "WELCOME",
                0,
                "SYSTEM",
                (int) generatedUid // Account Isolation (UID-based)
            );
            AppDatabase.getDatabase(this).notificationDao().insert(welcomeNotice);

            // Success: Start Isolated Session via SessionManager (Using UID anchor)
            SessionManager sessionManager = SessionManager.getInstance(this);
            sessionManager.startSession((int) generatedUid, email, username);
            
            sessionManager.putString("USER_USERNAME", username);
