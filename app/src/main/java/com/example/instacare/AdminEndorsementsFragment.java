...
public class AdminEndorsementsFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        String currentUserRole = sessionManager.getString("USER_ROLE", "ADMIN");
        
        recyclerView.setLayoutManager(new LinearLayoutManager(requireContext()));
        adapter = new EndorsementAdapter(new ArrayList<>(), currentUserId, currentUserRole, this::showReviewDialog, null);
        recyclerView.setAdapter(adapter);

        // Chip group filtering
        ChipGroup chipGroup = view.findViewById(R.id.chipGroup);
        
        // Set red tint for Admin empty icon
        android.widget.ImageView ivEmpty = view.findViewById(R.id.ivEmptyIcon);
        if (ivEmpty != null) {
            ivEmpty.setImageTintList(android.content.res.ColorStateList.valueOf(getResources().getColor(R.color.primary, null)));
        }
        
        // Apply red role colors and ripples to chips for Admin
        int activeColor = android.graphics.Color.parseColor("#DC2626"); // Admin Red
        int inactiveColor = android.graphics.Color.parseColor("#F1F5F9");
        int rippleColor = android.graphics.Color.parseColor("#30DC2626"); // Semi-transparent red
    ...
    private void showReviewDialog(Endorsement endorsement) {
        ...
        // Role-Based Button Coloring (Red for Admin)
        ...
        int primaryLight = android.graphics.Color.parseColor("#15D32F2F"); // Very light red for tonal button background
        ...
        switch (endorsement.status) {
            case "PENDING":
                ...
                actionButtons.setVisibility(View.GONE); // Admin no longer approves
                break;
            case "APPROVED":
                ...
                if (endorsement.adminRemarks != null && !endorsement.adminRemarks.isEmpty()) {
                    ...
                    remarksLayout.setHint("Processed By");
            ...
            case "DECLINED":
                ...
                if (endorsement.adminRemarks != null && !endorsement.adminRemarks.isEmpty()) {
                    remarksLayout.setVisibility(View.VISIBLE);
                    remarksLayout.setHint("Decision Reason");
                    editRemarks.setText(endorsement.adminRemarks);
                    editRemarks.setEnabled(false);
                }
                break;
        ...
        // Actions removed for Admin Role (View Only)
        dialogView.findViewById(R.id.btnApprove).setVisibility(View.GONE);
        dialogView.findViewById(R.id.btnDecline).setVisibility(View.GONE);
